...
- 🔧 Автоинициализация git-репозитория
- 🌐 Определение платформы (GitHub / GitLab / Bitbucket)
- ☁️ Автоматическое создание удалённого репозитория
- 🌀 Push в main + добавление в отслеживаемый список
- ⏱ Настройка cron-синхронизации через `update-all`
- 📁 Работа с шаблонами `.gitignore`, `README.md`
- 💬 Уведомления через `notify-send` (если доступно)
- ⚙️ Конфигурация через `~/.create-repo.conf`
...
Если не указано имя — используется текущая папка. Без параметров — создаёт репозиторий, пушит его и добавляет в cron.
...
| Флаг             | Назначение |
|------------------|------------|
| `--status`       | Показать статус cron и текущей папки |
| `--log [N]`      | Показать последние N строк из лога |
| `--clean`        | Удалить несуществующие папки из `.repo-autosync.list` |
| `--list`         | Список всех отслеживаемых репозиториев |
| `--remove`       | Удалить текущую папку из автослежения |
| `--update`       | Обновить до последней версии из GitHub |
| `--dry-run`      | Только показать действия без пуша |
| `--share`        | Показать ссылку на репозиторий и командную ссылку |
| `--contributors` | Показать всех участников проекта |
| `--team NAME`    | Указать команду GitHub по умолчанию |
...
Скрипт `update-all` запускается через cron и коммитит/пушит изменения в отслеживаемых проектах.

Лог: `~/.create-repo.log`  
Cron: `*/1 * * * * /usr/local/bin/update-all`
...
Если доступен `notify-send`, будут выводиться уведомления о синхронизации.

---

## 🐛 Лог ошибок

Ошибки команд записываются в:

```bash
~/.create-repo-errors.log
```
...
- `--pull-only` — только `git pull`
- Поддержка `.env` файлов
- Поддержка `main` / `master` веток
- Расширенная логика для `.create-repo-errors.log`

---

Хочешь ещё больше фич? Открой issue или используй `--interactive` 😉
